...
public abstract class WebUtils {
    ...
    static {
        ...
        verifier = new HostnameVerifier() {
            public boolean verify(String hostname, SSLSession session) {
                //return false;//默认认证不通过，进行证书校验。（生产环境用这个）
            	return true;//测试环境直接改为返回true
            }
        };
    ...
    public static String doPost2(String url, String reqBody,String charset, int connectTimeout,
            int readTimeout) throws IOException {
    	String ctype = "application/x-www-form-urlencoded;charset=" + charset;
        return doPost(url, ctype, reqBody.getBytes(), connectTimeout, readTimeout);
    }
    ...
    public static String doPost(String url, Map<String, String> params, String charset,
        ...
        System.out.println("post报文体==>"+query);
        byte[] content = {};
        if (query != null) {
    ...
    public static String doPost(String url, String ctype, byte[] content, int connectTimeout,
        ...
        try {
            try {
                ...
                if (unsafeHttps) { //不验服务器证书
                	HttpsURLConnection husn = (HttpsURLConnection) conn;
                    husn.setSSLSocketFactory(socketFactory);
                    husn.setHostnameVerifier(verifier);
                }
            } catch (IOException e) {
